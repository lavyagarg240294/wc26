...
- **Live match detail** — score + minute that **refresh live in place** (open a running game and it stays current — score, minute, timeline, stats — without reopening), a goal/card/sub **timeline** (tap any player for a mini profile — photo, club, caps, a **per-match box score** & what they did), **lineups on a formation pitch** (official player photos), a match-flow sparkline, a rich **match-stats** panel (possession, passing, tackles, saves, crosses… ~16 metrics) with **standout performers**, and match facts (attendance, referee).
...
- **Stats** (Players · Teams · Discipline · Records · Tournament · Rankings) — the **Golden Boot** & **assists** races, **goalkeepers/clean sheets**, a stack of **team leaderboards** (pass accuracy, defensive actions, crosses, saves…), per-team **playing-style fingerprints**, a **suspension watch** and **fair-play** table, **records** (biggest win, fastest/latest goal…), a **player head-to-head** compare, a **confederation** breakdown, and the **complete FIFA world ranking** (all 211 teams, filterable to the 48 finalists, with confederation tags).
