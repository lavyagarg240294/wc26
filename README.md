...
- **Brand & marks.** The site's own logo is the gold **centre-circle mark** (`assets/mark.svg`) — a pure-geometry pitch centre circle — reused for the favicon, the PWA/home-screen icons (`scripts/make-icons.py`), the social/OG card (`scripts/make-og.py`) and the in-app *About* sheets. The **FIFA World Cup 26™ emblem** and the **adidas Trionda** ball shown in the *About the tournament* sheet are official marks of FIFA/adidas, used for identification only; the Trionda photo is by Wikimedia Commons user *User34790*, [CC BY-SA 4.0](https://creativecommons.org/licenses/by-sa/4.0/).
