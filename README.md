...
A fast, no-build fan site that followed the **World Cup 2026** live from the first kickoff to the final - and now keeps it: **Spain's 1-0 a.e.t. win over Argentina** on July 19, and everything that led there, archived in full. Every match, lineup, stat, report and story from all 104 games, plus **The Wrap** - the tournament told from the data.

> **The tournament is over.** The site runs in a static archive mode: no live polling, no scheduled jobs - the baked JSON in `data/` *is* the site, permanently. Everything below describes both what the site did live and what it keeps now.

**What's inside**
- **The Wrap** - the tournament's story computed from the data at render time: the headline numbers (308 goals, 16 comeback wins, exactly 1,000 substitutions), **Spain's road** (never trailed for a single minute), the final and the ten-goal bronze final, goals by stage and minute, the comebacks and rank-gap upsets, the **honours board** (Golden Boot, assists, clean sheets, physical, hat-tricks), and the prediction model's own **scorecard** - a leave-prior replay of the production model across all 104 matches (`scripts/make-backtest.mjs` → `data/backtest.json`), misses included.
- **Matches** - every fixture with kickoff times on your local clock, final scores, a champions hero card, and a **Day in review** digest, navigable by date.
- **Open any match** (live or finished) - score + minute that **refresh live in place** (open a running game and it stays current - score, minute, timeline, stats - without reopening; the win probability leads while it's live), a **win probability + predicted scoreline** for the match (a Dixon-Coles model over an xG-aware Elo that updates after every result, refined by an attack-vs-defence read, host edge and late-group qualification stakes - with the reasoning shown), a goal/card/sub **timeline** (tap any player for a profile - photo, **age · height · weight**, club, caps, their **match log** + a per-match box score & what they did), **lineups on a formation pitch** (official player photos), a rich **match-stats** panel **grouped by attack / possession / defence** (possession, passing, tackles, saves, crosses… ~16 metrics) with **standout performers**, a derived **match-control index** (xG + shots + on-target + corners), a live **Physical** read (distance, top speed & sprints, updating in place while a match runs), FIFA's post-match **deep analysis** (official xG, per-player **distance covered, top speed & sprints**, phases of play), and match facts (attendance, referee).
- **Teams** - an all-teams landscape of **what changed since Qatar 2022** (32 → 48: who's back, who's on debut), with the **full Qatar 2022 results** (every match, score & scorers) one tap away - or flip the grid to a **world map** with all 48 nations pinned where they sit on the globe. Tap any team anywhere for a detail sheet: overview, its **country outline + population**, recent form, every fixture, the group standing + qualification outlook, a one-tap follow (re-themes the UI to their kit - contrast-guarded so it always stays readable), **squad rotation & minutes**, **who's continuing from the 2022 squad vs new**, the full squad (with full-photo cards for your team), **"road to the final"** (a projected knockout route from live standings), its **World Cup penalty shoot-out record** (won-lost across every WC shoot-out that nation has faced since 1982, this tournament included), and a side-by-side **compare** against any other team.
- **Bracket** - the settled knockout tree, R32 → Final, winners advancing round by round, with the final group tables and the best-third race one tap away. (During the groups it led with the 12 live tables and a "what each team needs" outlook.)
- *Retired with the tournament:* the **Field** (the knockout-stage centre that counted sixteen down to one - its slot now hosts The Wrap) and **Predict** (the three-scenario knockout predictor - its bracket entry points are hidden and old share links land gracefully). Both stay dormant in `app.js` rather than deleted.
- **Stats** (Overview · Goals · Players · Teams · Compare · Ranking · All-time) - a tournament **overview** (goals/match, **shots on target, penalties & own goals**, records so far: biggest win, fastest/latest goal, and a **confederation table** with each region's record + goals); a **Goals** section - a **"when goals are scored"** timing chart (five-minute bands out to 120', with a separate bar for each period's stoppage time), a filterable **"where shots are taken"** shot map (by round, by team, shots-taken vs shots-faced, by scorer) and a **"where goals cross the line"** net-placement map; the **Golden Boot** & **assists** races, **goalkeepers/clean sheets**, **physical leaderboards** from FIFA's post-match data (**distance covered**, **sprints** and **top speed**, total and per-match), a stack of **team leaderboards** (pass accuracy, defensive actions, crosses, saves…) and per-team **playing-style fingerprints** (with a per-metric explainer) - with a **Quarter-finalists** scope toggle on the player & team boards once the last eight are set; a **suspension watch** and **fair-play** table, **all-time World Cup leaderboards** (career goals, appearances, World Cups played, titles… with this tournament's players tracked live as they close on the marks), a **player head-to-head** compare, and the **complete FIFA world ranking** (all 211 teams, filter to the 48 **WC qualifiers** and/or by confederation).
- **Players** - browse all **1,248 squad players** in one place: a fast search (name, club or country), a **position** filter, searchable **country** and **club** pickers (the club one ranked by how many players each club sends), and a two-way sort by **caps or age**. Captains are flagged. Tap anyone for the full profile (a **tap-to-enlarge** photo, **age · height · weight**, club, caps & career goals, match log, and a **Match stats** block from FIFA's physical data - distance, top speed, sprints, shots on target, xG and passing).
...
- Plus: **global search** (⌘K - teams, players, matches), **installable to your home screen** (PWA), **dark mode**, an optional **animated background** (suspended gold glitter; off by default), an optional **stadium-mix** soundtrack, **match alerts** (goal/kickoff notifications) and **data-saver** mode, a **Settings** sheet, per-match **calendar** export (.ics, client-side), per-match **share cards** that unfurl on social, an accessibility pass (keyboard focus, reduced-motion, screen-reader live goals), and a fully mobile-first, re-themable UI.

**Stack:** plain `index.html` + `styles.css` + one vanilla-JS `app.js` on GitHub Pages. While the tournament ran, the **live score, minute, timeline and line-ups were read straight from FIFA's public feed by the browser** (real-time, nothing in between), with everything slower baked into JSON by GitHub Actions. Now the tournament-over flag stops all polling and the baked JSON is the whole site. **No API keys were ever required** - all live data came from free, keyless sources. Total cost: **$0**.
...
## After the final (July 19) - done

This shutdown has been carried out. All six scheduled workflows (buzz, EFI, playerstats, shots, health, and the scores-loop restarter) had their cron schedules removed on 2026-07-20, keeping `workflow_dispatch` for manual re-runs; the running scores loop was cancelled. In the app, a latched `tournamentOver()` flag (the final is feed-confirmed) switches off FIFA polling, the ticker, countdowns and kickoff alerts - after a visitor's first load, the site makes no live calls at all. The data was verified complete first: all 104 matches with events, lineups, stats, shots, xG and physical data, every timeline summing exactly to its recorded score. The site keeps working indefinitely as a frozen record of the tournament.
