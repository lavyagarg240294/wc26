...
- **Tables** — 12 live group tables + a "what each team needs to qualify" outlook, a **third-place race** tracker (the best-8-of-12 cut line the new format needs), and the live **projected Round of 32 board** ("as it stands" — big-flag tie cards, each marked Confirmed / N games left / spot-still-open).
- **Predict** — opens on the live **projected Round of 32 "as it stands"**: big-flag tie cards from current standings, each marked **Confirmed / N games left / spot still open** (undecided third-place slots stay honestly blank rather than guessing a nation). Then build up to **three what-if scenarios**: order every group, pick the best third-placed teams, then tap winners through the full knockout bracket to crown your champion (seeded from live standings so it's ready to play with right away), each with its own **path to glory**; see **how your call is tracking vs reality**, and **share a scenario as a ~30-character link**.
