...
- **Field** - a knockout-stage "centre" that counts the field down from sixteen to one: every surviving team a kit-colour **monolith** in bracket order on a dark broadcast stage, a sticky **stage bar** (the next kickoff's countdown, or the live score), and a wall the tournament demolishes round by round - a **goal shockwave** crossing the panels as a goal lands, an **elimination ritual** as each side falls, then a **departures board** and a **departed shelf** where nobody leaves the page (tap a fallen team for its whole run and the way it ended). Per round: an **editorial single-pager** on each tie - what the matchup means, both sides' road here, their past World Cup meetings, honest about its own age once the tie settles - and **star-player** profiles, written and fact-checked.
- **Bracket** - during the group stage it leads with the **12 live group tables** + a "what each team needs to qualify" outlook and a **third-place race** tracker (the best-8-of-12 cut line the new format needs), with the live **knockout bracket "as it stands"** filling in beneath. Once the groups finish, the **live knockout bracket leads** - winners advancing round by round, R32 → Final - and the final tables and best-third race fold one tap away.
- **Predict** - grouped in the nav with **Field** and **Bracket**. Opens on the live **knockout bracket "as it stands"** as a converging tree: both halves of the draw funnel to a central final, the **Round of 32 on the outer edges**, winners advancing as matches are played. Tap a round to trace how it forms, or any tie for the **projected matchup, date & venue, the model's odds, and the two sides' past World Cup meetings** (every meeting since 1930, with the result). Then build up to **three what-if scenarios**: order every group, pick the best third-placed teams, then **tap winners through an interactive converging bracket** - each round collapses inward (32 → 16 → 8 → 4 → 2 → 1) toward the trophy until you crown your champion (seeded from live standings so it's ready to play with right away), each with its own **path to glory**. Share your prediction three ways: a **~30-character deep-link**, a **champion card**, or the **full predicted-bracket card** (every pick, both halves funnelling to your winner).
