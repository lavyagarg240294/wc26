...
const BUILD = "491";  // shown in footer; bump with the ?v= asset version
...
function pitchSide(side, s, home, full) {
  ...
  const dot = (p, x, depth, slot) => {                                  // depth 0 = own goal, 1 = halfway; slot = the % of pitch width this player owns horizontally
    ...
    return `<div class="pp pp-clk${photo ? "" : " pp-nf"}${p[3] != null ? " pp-sub" : ""}" data-player="${esc(p[1])}|${s.code}" role="button" tabindex="0" style="left:${left}%;top:${top}%;width:${slot.toFixed(1)}%;z-index:${Math.round(100 - top)};--pc:${c1};--pt:${c2};--pno:${pno}">${face}${p[3] != null ? `<i class="pp-subdot" title="On${p[3] ? " " + Math.floor(p[3]) + "'" : ""}"></i>` : ""}<span class="pp-name">${num !== "" ? `<b class="pp-no">${num}</b>` : ""}<span class="pp-nm">${esc(lastName(p[1]))}</span></span></div>`;
  };
...
// the XI at a moment: start from the announced line-up and apply substitutions in order - up to `upTo` minutes for a
// live match (so it tracks the game in real time), or all of them once it's finished. Players are matched by jersey
// number (robust to name casing), and the substitute inherits the slot so the shape holds. p[3] carries the on-minute.
function applyLineup(side, tm, code, r, upTo) {
  const xi = side.xi.map(p => p.slice());
  const norm = s => _deburr(s || "").replace(/[^a-z]/g, "");
  for (const e of (r.ev || [])) {
    if (e.k !== "S" || e.tm !== tm || !e.on || !e.off) continue;
    const mn = _tlMin(e.t);
    if (upTo != null && mn > upTo) continue;               // hasn't happened yet (live)
    const offP = resolvePlayer(e.off, code), onP = resolvePlayer(e.on, code);
    let idx = offP ? xi.findIndex(p => p[0] === offP.n) : -1;
    if (idx < 0) idx = xi.findIndex(p => norm(p[1]) === norm(e.off));   // fall back to a name match
    if (idx < 0) continue;
    xi[idx] = [onP?.n ?? "", e.on, xi[idx][2], mn];         // keep the slot's role/position; stash the on-minute
  }
  return { f: side.f, coach: side.coach, xi };
}
...
function pitchMarks(side, s, home) {
  ...
  const add = (p, x, depth) => out.push({ x: home ? x : 100 - x, y: home ? 92 - depth * 80 : 8 + depth * 80, num: p[0] ?? "", name: p[1] || "", code: s.code, home, sub: p[3] != null, subMin: p[3] });
  add(fr.gk, 50, 0.06);
...
function separateMarks(marks, min) {
  ...
  for (let it = 0; it < 80; it++) {
    for (let i = 0; i < P.length; i++) for (let j = i + 1; j < P.length; j++) {
      ...
      // both names sit BELOW their disc, so two markers in nearly the same column need extra vertical room or the
      // upper one's name lands on the lower disc. Enforce a taller minimum gap when they're roughly stacked.
      else if (Math.abs(b.x - a.x) < 8) { const gy = b.y - a.y, ag = Math.abs(gy) || 0.01; if (ag < 19) { const p = (19 - ag) / 2 * (gy < 0 ? -1 : 1); a.y -= p; b.y += p; } }
    }
...
function pitchOverlay(xiH, sH, xiA, sA) {
  ...
  if (!mh || !ma) return "";
  const all = [...mh, ...ma]; separateMarks(all, 13.5);
  return all.map(m => {
    const c1 = (m.code && S.teams[m.code]?.c1) || "#334155", dt = relLum(c1) > .5 ? "#14181C" : "#ffffff";
    return `<div class="ppd${m.home ? " ppd-h" : " ppd-a"}${m.sub ? " ppd-sub" : ""}" data-player="${esc(m.name)}|${m.code}" role="button" tabindex="0" aria-label="${esc(m.name)}${m.sub ? `, on ${m.subMin ? Math.floor(m.subMin) + "'" : "as sub"}` : ""}" style="left:${m.x.toFixed(1)}%;top:${m.y.toFixed(1)}%;z-index:${Math.round(200 - m.y)};--pc:${c1};--pt:${dt}"><span class="ppd-disc">${m.num}${m.sub ? `<i class="ppd-subdot" title="On${m.subMin ? " " + Math.floor(m.subMin) + "'" : ""}"></i>` : ""}</span><span class="ppd-nm">${esc(lastName(m.name))}</span></div>`;
  }).join("");
}
...
let _xiLineup = "start";  // starting XI vs the players on the pitch now (subs applied)
function xiPanel(m, h, a) {
  const r = res(m), xi = r?.xi; if (!xi) return "";
  if (formationRows(xi.h) && formationRows(xi.a)) {
    const st = status(m), done = isFinalSt(st), live = st === ST.LIVE || st === ST.HT;
    const hasSubs = (r.ev || []).some(e => e.k === "S" && (e.tm === "h" || e.tm === "a") && e.on && e.off);
    const upTo = done ? 999 : Math.max(0, st === ST.HT ? 45 : (r.min != null ? _tlMin(r.min) : 0));   // apply subs up to the current minute (all, when finished)
    const now = _xiLineup === "now" && hasSubs;
    const xh = now ? applyLineup(xi.h, "h", h.code, r, upTo) : xi.h;
    const xa = now ? applyLineup(xi.a, "a", a.code, r, upTo) : xi.a;
    const tag = (s, side) => `<span class="pitch-team"><span class="fl">${s.code ? flag(s.code) : ""}</span>${esc(s.name)}${side.f ? ` <b>${esc(side.f)}</b>` : ""}</span>`;
    const lineSeg = hasSubs ? `<div class="xi-seg xi-seg-line" role="tablist" aria-label="Line-up snapshot">
      <button type="button" class="xi-seg-b${!now ? " on" : ""}" data-xiline="start" role="tab" aria-selected="${!now}">Starting XI</button>
      <button type="button" class="xi-seg-b${now ? " on" : ""}" data-xiline="now" role="tab" aria-selected="${now}">${live ? "On the pitch now" : "Playing XI"}</button>
    </div>` : "";
    const seg = `<div class="xi-seg" role="tablist" aria-label="Which team">
    ...
    const body = _xiView === "home" ? pitchSide(xh, h, true, true)
      : _xiView === "away" ? pitchSide(xa, a, false, true)
      : pitchOverlay(xh, h, xa, a);
    const hc = S.teams[h.code]?.c1 || "#334155", ac = S.teams[a.code]?.c1 || "#334155";
    const legend = _xiView === "both" ? `<div class="xi-legend"><span class="xi-lg"><i class="xi-sw-solid" style="background:${hc}"></i>${esc(cname(h.code) || h.name)}</span><span class="xi-lg"><i class="xi-sw-ring" style="border-color:${ac}"></i>${esc(cname(a.code) || a.name)}</span>${now ? `<span class="xi-lg"><i class="xi-sw-sub"></i>came on</span>` : ""}</div>` : "";
    return `<div class="xi-wrap" data-mid="${esc(m.id)}">
...
function openMatch(id, reuse) {   // reuse: re-render the body in place (a live poll) without re-animating the dialog
  ...
  const pXiInline = r?.xi ? `<div class="eyebrow">${liveNow ? "Line-ups" : "Starting XI"}</div>${xiPanel(m, h, a)}` : "";
  const pXiFold = r?.xi ? `<details class="md-fold"><summary><span>${liveNow ? "Line-ups" : "Starting XI"}</span><small>${esc([r.xi.h?.f, r.xi.a?.f].filter(Boolean).join(" v ")) || "line-ups & formations"}</small></summary><div class="md-fold-body">${xiPanel(m, h, a)}</div></details>` : "";
...
// a team's group-qualification status, shared by the group tables AND the group-match cards (one source of truth).
// "in" = through to the last 32, "out" = can't make the top two, "" = undecided / not started / a 3rd-placed team
// still in the best-third race. Group FINISHED → read off the final table (GD has settled any points ties: top two
// through, 4th out, 3rd left to the best-third race); IN PROGRESS → the GD-safe points scan (no GD-dependent call).
...
function tournamentStats() {
  ...
  const goalHist = new Array(28).fill(0), goalHalves = { h1: 0, h2: 0, et: 0 };   // when goals land: five-minute bands across 1–120 PLUS a dedicated stoppage bar after each period (45+, 90+, 105+, 120+); and the half split
  ...
  const goalBin = t => {   // 28 bins; a "+" minute (45+2, 90+3, 105+1, 120+4) goes to its period's dedicated stoppage bar
    const base = Math.floor(evMin(t)), plus = String(t).includes("+");
    if (plus) return base <= 45 ? 9 : base <= 90 ? 19 : base <= 105 ? 23 : 27;   // 45+, 90+, 105+, 120+ stoppage bars
    if (base <= 45) return Math.max(0, Math.floor((base - 1) / 5));               // 1–45  → bins 0–8
    if (base <= 90) return 10 + Math.floor((base - 46) / 5);                      // 46–90 → bins 10–18
    if (base <= 105) return 20 + Math.floor((base - 91) / 5);                     // 91–105 → bins 20–22
    return Math.min(26, 24 + Math.floor((base - 106) / 5));                       // 106–120 → bins 24–26
  };
...
function goalTimingHTML(s) {
  ...
  if (!total) return "";
  // 28 bins: five-minute bands across 1–120, with a dedicated stoppage bar at the end of each period. [tooltip, axis label, isStoppage]
  const BINS = [
    ["1–5"], ["6–10"], ["11–15", "15"], ["16–20"], ["21–25"], ["26–30", "30"], ["31–35"], ["36–40"], ["41–45"], ["45+ stoppage", "45+", 1],
    ["46–50"], ["51–55"], ["56–60", "60"], ["61–65"], ["66–70"], ["71–75", "75"], ["76–80"], ["81–85"], ["86–90"], ["90+ stoppage", "90+", 1],
    ["91–95"], ["96–100"], ["101–105"], ["105+ stoppage", "105+", 1], ["106–110"], ["111–115"], ["116–120"], ["120+ stoppage", "120+", 1],
  ];
  const N = BINS.length, peak = h.indexOf(max), pk = BINS[peak] || ["", ""];
  const peakLbl = pk[2] ? `in ${pk[1]} added time` : `between minutes ${pk[0] || "?"}`;
  const bars = h.map((c, i) => { const b = BINS[i] || ["", ""], stop = b[2] === 1;
    return `<div class="gt-bar${stop ? " gt-bar-x" : ""}" title="${c} goal${c === 1 ? "" : "s"} · ${b[0]}${stop ? "" : "'"}"><span class="gt-fill" style="height:${Math.round(c / max * 100)}%"></span><span class="gt-x">${b[1] || ""}</span></div>`; }).join("");
  const hg = s.goalHalves || { h1: 0, h2: 0, et: 0 };
  const div = (i, cls) => `<span class="gt-ht${cls ? " " + cls : ""}" style="left:${(i / N * 100).toFixed(2)}%"></span>`;   // half-time (after 45+) and full-time (after 90+); the extra-time halves are bounded by the gold 105+/120+ bars
  return `<div class="eyebrow">When goals are scored ${infoBtn("Every goal placed by the minute it was scored, in five-minute bands across the 90 and through extra time (120'), with a SEPARATE bar for each period's added (stoppage) time — 45+, 90+, 105+ and 120+ — rather than folding those into the regular bands. A read on when goals have actually come, not a prediction.", "How this is counted")}</div>
    <div class="gt-card"><div class="gt-chart" role="img" aria-label="Goals by five-minute band">${bars}${div(10)}${div(20, "gt-ft")}</div>
...
function renderStats() {
  ...
  wireCompareTable(el);   // sort/filter/lens controls for the team-comparison table
}
...
/* ============================ team-comparison table (Stats › Compare) ============================ */
// Per-team aggregation across every FEED-FINAL match a side has completed. res(m).stats values are [home, away];
// slotInfo says which index a team is. Counts are summed; possession & pass accuracy are averages/ratios.
function teamStatsAgg() {
  const A = {}, K = ["poss", "sh", "sot", "blk", "sv", "off", "cor", "fls", "pass", "passT", "cross", "lball", "tkl", "intc", "clr", "yc", "rc"];
  const ini = c => (A[c] = { code: c, mp: 0, sp: 0, gf: 0, ga: 0, w: 0, d: 0, l: 0, poss: 0, sh: 0, sot: 0, blk: 0, sv: 0, off: 0, cor: 0, fls: 0, pass: 0, passT: 0, cross: 0, lball: 0, tkl: 0, intc: 0, clr: 0, yc: 0, rc: 0 });
  for (const m of S.matches) {
    if (!isFeedFinal(m)) continue;
    const hc = slotInfo(m, "home").code, ac = slotInfo(m, "away").code, r = res(m);
    if (!hc || !ac || !r || r.h == null) continue;
    A[hc] || ini(hc); A[ac] || ini(ac);
    A[hc].mp++; A[ac].mp++;
    A[hc].gf += r.h; A[hc].ga += r.a; A[ac].gf += r.a; A[ac].ga += r.h;
    if (r.h > r.a) { A[hc].w++; A[ac].l++; } else if (r.h < r.a) { A[ac].w++; A[hc].l++; } else { A[hc].d++; A[ac].d++; }
    const s = r.stats;
    if (s && s.poss) { A[hc].sp++; A[ac].sp++; K.forEach(k => { if (s[k]) { A[hc][k] += s[k][0]; A[ac][k] += s[k][1]; } }); }
  }
  return Object.values(A);
}
// Which R32 route a team took: "top2" (finished 1st/2nd - the proper way in), "third" (one of the eight best
...
const _cGoals = (k, label, t) => ({ k, label, t, num: (r, p) => p ? (r.mp ? r[k] / r.mp : 0) : r[k], cell: (r, p) => p ? _fx1(r[k] / r.mp) : r[k] });
const _cPoss = { k: "poss", label: "Poss", t: "Average possession", num: r => r.sp ? r.poss / r.sp : 0, cell: r => r.sp ? (r.poss / r.sp).toFixed(1) + "%" : "—" };
const _cPacc = { k: "pacc", label: "Pass%", t: "Pass accuracy (accurate ÷ attempted)", num: r => r.passT ? 100 * r.pass / r.passT : 0, cell: r => r.passT ? Math.round(100 * r.pass / r.passT) + "%" : "—" };
const _cGd = { k: "gd", label: "GD", t: "Goal difference", num: (r, p) => (p ? (r.mp ? (r.gf - r.ga) / r.mp : 0) : (r.gf - r.ga)), cell: (r, p) => { const v = p ? (r.gf - r.ga) / r.mp : (r.gf - r.ga); return (v > 0 ? "+" : v < 0 ? "−" : "") + (p ? _fx1(Math.abs(v)) : Math.abs(v)); } };
const _cSotp = { k: "sotp", label: "SOT%", t: "Shots on target ÷ shots", num: r => r.sh ? 100 * r.sot / r.sh : 0, cell: r => r.sh ? Math.round(100 * r.sot / r.sh) + "%" : "—" };
const _cMp = { k: "mp", label: "MP", t: "Matches played", num: r => r.mp, cell: r => r.mp };
const CMP_LENSES = [
  ["overview", "Overview", [_cMp, _cGoals("gf", "GF", "Goals for"), _cGoals("ga", "GA", "Goals against"), _cGd, _cPoss, _cCount("sh", "Sh", "Shots"), _cPacc]],
  ["attack", "Attack", [_cGoals("gf", "GF", "Goals for"), _cCount("sh", "Sh", "Shots"), _cCount("sot", "SOT", "Shots on target"), _cSotp, _cCount("cor", "Cor", "Corners"), _cCount("cross", "Cross", "Crosses")]],
  ["passing", "Passing", [_cCount("pass", "Pass", "Accurate passes"), _cPacc, _cPoss, _cCount("lball", "Long", "Long balls"), _cCount("cross", "Cross", "Crosses")]],
  ["defence", "Defence", [_cGoals("ga", "GA", "Goals against"), _cCount("tkl", "Tkl", "Tackles"), _cCount("intc", "Int", "Interceptions"), _cCount("clr", "Clr", "Clearances"), _cCount("blk", "Blk", "Blocks"), _cCount("sv", "Sv", "Saves")]],
...
async function boot() {
  ...
  document.addEventListener("click", e => {
    ...
    const xseg = e.target.closest("[data-xiview],[data-xiline]");   // pitch toggles (team view / starting vs playing XI) - swap in place, no modal re-render
    if (xseg) { e.stopPropagation();
      if (xseg.dataset.xiview) { if (xseg.dataset.xiview === _xiView) return; _xiView = xseg.dataset.xiview; }
      else { if (xseg.dataset.xiline === _xiLineup) return; _xiLineup = xseg.dataset.xiline; }
      const wrap = xseg.closest(".xi-wrap"), m = wrap && S.matches.find(x => x.id === wrap.dataset.mid);
      if (wrap && m && res(m)?.xi) { const tmp = document.createElement("div"); tmp.innerHTML = xiPanel(m, slotInfo(m, "home"), slotInfo(m, "away")); if (tmp.firstElementChild) wrap.replaceWith(tmp.firstElementChild); }
      return; }
