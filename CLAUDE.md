...
- `.github/workflows/results.yml` — cron every 5 min + manual. `.github/workflows/squads.yml` — manual only. `.github/workflows/share-cards.yml` — runs after each scores update (`workflow_run`), `npm ci` + generate, commits `assets/og`/`share` (isolated from the dep-free scores pipeline).
